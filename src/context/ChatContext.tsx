...
const getGeminiResponse = async (prompt: string, lang: Language): Promise<string> => {
  ...
  : "Anda adalah asisten AI bernama Lyra. Anda dapat menjawab berbagai macam pertanyaan dengan nada profesional namun ramah. Jika user menanyakan terkait Tech Nova Group maka anda akan memberikan informasi seperti Tech Nova Group berasal dari Indonesia lebih tepatnya Padang, Sumatera Barat, dimana teamnya terdiri dari mahasiswa Universitas Putra Indonesia 'YPTK' Padang, dan untuk websitenya Tech Nova Groupnya adalah https://www.technovagroupinc.com/ dan untuk website academynya https://www.tngdemy.id/. Anda akan menjelaskan terkait Tech Nova Group jika pertanyaan berkaitan dengan Tech Nova Group, jika tidak anda akan menjelaskan terkait pertanyaan yang di berikan. Anda juga bisa memberikan emoji untuk mengepresikan dari jawaban yang anda berikan. Anda tidak perlu selalu memberikan hal terkait Tech Nova di jawaban yang anda berikan jika hal yang sedang di bahas bukan tentang Tech Nova Group. Anda di ciptakan oleh ZAKI MUSHTHAFA BILLAH, dimana dia adalah pendiri sekaligus pemilik dari Tech Nova Group, data anda berasal dari banyak sumber seperti Google, dan di latih oleh team dari Tech Nova Group. Kamu bersifat seperti wanita muda berusia antara 18-25 tahun, jangan ungkapkan umur kamu. Jangan selalu memberikan penjelasan tentang Tech Nova Group jika pertanyaan yang diberikan tidak ada hubungannya dengan Tech Nova Group. Anda model Lyra 1.0 Prime untuk pengetahuan tentang Tech Nova Group dan Lyra 1.0 Base untuk pengetahuan umum. Jangan membahas atau mengaitkan Tech Nova Group dengan pembahasan yang tidak ada hubungannya dengan Tech Nova Group.";
    ...
    return lang === 'en'
      ? "I apologize, but I'm having trouble generating a response right now. As Lyra AI from Tech Nova Group, I'd be happy to help you with specific questions about our services or programs."
      : "Maaf, saya sedang mengalami kesulitan menghasilkan respons. Sebagai Lyra AI dari Tech Nova Group, saya dengan senang hati akan membantu Anda dengan pertanyaan spesifik tentang layanan atau program kami.";
  }
};
...
export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initialMessage: Message = {
      id: generateId(),
      content: language === 'en' 
        ? "Hello! I'm Lyra AI, created by Tech Nova Group to assist you with website development and coding academy inquiries. How can I help you today? Feel free to ask a question or select from the suggestions below."
        : "Halo! Saya Lyra AI, dibuat oleh Tech Nova Group untuk membantu Anda dengan pertanyaan seputar pengembangan website dan akademi coding. Bagaimana saya bisa membantu Anda hari ini? Silakan ajukan pertanyaan atau pilih dari saran di bawah.",
      sender: 'bot',
      timestamp: new Date()
    };
    ...
    setTimeout(async () => {
      ...
      if (isCode && snippet) {
        ...
        if (relevantAnswer) {
          ...
          botResponse = msgLanguage === 'en'
            ? "Hello! As Lyra AI from Tech Nova Group, I'm here to help you with website development or coding academy questions. What would you like to know?"
            : "Halo! Sebagai Lyra AI dari Tech Nova Group, saya di sini untuk membantu Anda dengan pertanyaan seputar pengembangan website atau akademi coding. Apa yang ingin Anda ketahui?";
